<div class="py-20 px-6">
    <div class="max-w-7xl mx-auto">
        ...
        @if (session()->has('success'))
            <div class="mb-6 max-w-2xl mx-auto rounded-xl border border-green-200 bg-green-50 text-green-800 px-4 py-3 text-sm">
                {{ session('success') }}
            </div>
        @endif
        ...
        @else
            <p class="text-center text-lg opacity-70 mb-12 max-w-2xl mx-auto">Jelajahi koleksi lengkap pakaian olahraga unisex kami.</p>
        @endif

        <div class="max-w-3xl mx-auto mb-8">
            <input type="text" placeholder="Cari produk..." wire:model.live="search" class="w-full px-5 py-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-950" />
        </div>

        <div class="flex flex-wrap justify-center gap-3 mb-12">
            <button wire:click="setCategory('')" class="px-6 py-2 rounded-full font-medium transition-all {{ $category === '' ? 'bg-gray-950 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800' }}">Semua Produk</button>
            @foreach($categories as $cat)
                <button wire:key="cat-{{ $cat->id }}" wire:click="setCategory('{{ $cat->slug }}')" class="px-6 py-2 rounded-full font-medium transition-all {{ $category === $cat->slug ? 'bg-gray-950 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800' }}">{{ $cat->name }}</button>
            @endforeach
        </div>
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <svg viewBox="0 0 300 400" class="w-full h-full">
                                        ...
                                        <rect x="110" y="160" width="80" height="180" rx="10" fill="currentColor" opacity="0.15" />
                                    </svg>
                                </div>
                            @endif
                        </div>
                ...
                <div class="p-6 md:p-8">
                    <div class="flex items-start gap-4 mb-6">
                        <div class="w-24 h-32 rounded-xl overflow-hidden bg-gray-100 shrink-0">
                            ...
                            @endphp
                            @if($selImg)
                                <img src="{{ $selImg }}" alt="{{ $selectedProduct->name ?? 'Produk' }}" class="w-full h-full object-cover">
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <svg viewBox="0 0 300 400" class="w-full h-full">
                                        <rect width="300" height="400" fill="currentColor" opacity="0.08" />
                        ...
                        <div class="min-w-0">
                            <h3 class="font-semibold text-lg leading-tight">{{ $selectedProduct->name ?? 'Produk' }}</h3>
                            <p class="text-sm text-gray-500">Silakan isi formulir di bawah untuk mengirimkan inquiry.</p>
                        </div>
                        <button class="ml-auto text-gray-400 hover:text-gray-600" wire:click="closeInquiry" aria-label="Tutup">
                            ✕
                        </button>
                    </div>
                    ...
                    <form wire:submit.prevent="submitInquiry" class="grid gap-4">
                        <div>
                            ...
                            <input type="text" wire:model.defer="customer_name" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-900">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model.defer="customer_contact" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-900">
                            @error('customer_contact')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Pesan</label>
                            <textarea rows="4" wire:model.defer="message" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-900" placeholder="Saya tertarik dengan produk ini..."></textarea>
                            @error('message')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        @error('selectedProductId')<p class="text-sm text-red-600">{{ $message }}</p>@enderror

                        <div class="flex items-center justify-end gap-3 pt-2">
                            <button type="button" class="px-4 py-2 rounded-xl border border-gray-200" wire:click="closeInquiry">Batal</button>
                            <button type="submit" class="px-5 py-2 rounded-xl bg-gray-950 text-white">Kirim Inquiry</button>
                        </div>
                    </form>
                </div>
            </div>
