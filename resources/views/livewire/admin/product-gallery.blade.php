<div class="p-6 max-w-7xl mx-auto">
    ...
    @if($selectedProduct)
        <div class="mb-4 flex items-center justify-between">
            ...
            <button type="button" wire:click="closeProductGallery"
                    class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 cursor-pointer touch-manipulation">
                    ...
                    <div class="group relative bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="aspect-square relative">
                            ...
                            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                <button type="button" wire:click="setPrimary({{ $image['id'] }})"
                                        ...
                                        class="p-2 bg-white rounded-full hover:bg-gray-100 cursor-pointer touch-manipulation {{ $image['is_primary'] ? 'opacity-50 cursor-not-allowed' : '' }}"
                                        title="Jadikan Utama">
                                    <svg class="w-4 h-4 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                    </svg>
                                </button>
                                <button type="button" wire:click="openEditModal({{ $image['id'] }})"
                                        wire:loading.attr="disabled"
                                        class="p-2 bg-white rounded-full hover:bg-gray-100 cursor-pointer touch-manipulation"
                                        title="Edit">
                                    <svg class="w-4 h-4 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button type="button" wire:click="deleteImage({{ $image['id'] }})"
                                        ...
                                        class="p-2 bg-red-500 rounded-full hover:bg-red-600 cursor-pointer touch-manipulation"
                                        title="Hapus">
                        ...
                        <div class="p-3 flex items-center justify-between">
                            ...
                            <div class="flex gap-1">
                                <button type="button" wire:click="moveUp({{ $image['id'] }})"
                                        wire:loading.attr="disabled"
                                        class="p-1 hover:bg-gray-100 rounded cursor-pointer touch-manipulation">
                                    <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
                                    </svg>
                                </button>
                                <button type="button" wire:click="moveDown({{ $image['id'] }})"
                                        wire:loading.attr="disabled"
                                        class="p-1 hover:bg-gray-100 rounded cursor-pointer touch-manipulation">
                                    <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-6">
                    ...
                    <div class="mt-6 flex gap-3">
                        <button type="button" wire:click="closeEditModal"
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 cursor-pointer touch-manipulation">
                            Batal
                        </button>
                        <button type="button" wire:click="saveEdit"
                                wire:loading.attr="disabled"
                                class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 cursor-pointer touch-manipulation">
                            Simpan
                        </button>
                    </div>
                </div>
