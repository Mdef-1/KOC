        ...
        <div class="min-w-full overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-right text-gray-500 uppercase">Items Count</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($categories as $category)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-mono text-gray-600 bg-gray-100 px-2 py-1 rounded">{{ $category->slug }}</span>
                                </td>
            ...
            @else
                <div class="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Add categories to organize your products.</p>
                </div>
            @endif
        </div>
    </div>
                ...
                <div class="relative inline-block w-full max-w-lg overflow-hidden text-left align-middle transition-all transform bg-white rounded-xl shadow-2xl">
                    <div class="px-6 py-4 border-b border-gray-100">
                        <h3 class="text-lg font-bold text-gray-900">
                            {{ $selected_id ? 'Edit' : 'Create' }} Category
                        </h3>
                    </div>
                    ...
                    <form wire:submit.prevent="store">
                        <div class="px-6 py-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model.defer="slug"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50 sm:text-sm" placeholder="auto-generated-if-blank">
                            </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <label for="is_active" class="ml-2 block text-sm text-gray-900 font-medium">Active Category</label>
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-4 bg-gray-50 flex justify-end space-x-3">
                            ...
                            <button type="submit" wire:loading.attr="disabled" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 disabled:opacity-50">
                                <span wire:loading.remove>{{ $selected_id ? 'Save Changes' : 'Create Category' }}</span>
